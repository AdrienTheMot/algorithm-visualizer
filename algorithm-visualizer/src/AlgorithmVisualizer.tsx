...
function recordSteps<T>(arr: T[], steps: T[][]) {
  steps.push([...arr]);
}
...
export default function AlgorithmVisualizer() {
  const [data, setData] = useState<number[]>(
    () => Array.from({ length: 30 }, () => randInt(5, 100))
  );
  ...
  useEffect(() => {
    setData(Array.from({ length: size }, () => randInt(5, 100)));
  }, [size]);
        ...
        <section className="grid md:grid-cols-2 gap-6">
          <div className="p-4 rounded-2xl shadow bg-white">
            ...
            <div className="flex flex-wrap gap-3 items-center">
              <label className="text-sm">
                ...
                <select
                  ...
                  value={algo}
                  onChange={(e) => setAlgo(e.target.value as keyof typeof SORTERS)}
                >
            ...
            <p className="mt-2 text-xs text-gray-500">
              Tip: Increase speed for faster animations; decrease for step-by-step.
            </p>
          </div>
          ...
          <div className="p-4 rounded-2xl shadow bg-white">
            ...
            <div className="flex flex-wrap items-center gap-3">
              <input
                ...
                placeholder="Enter numbers e.g. 8,3,10,1,6,14,4,7,13"
                value={bstInput}
                onChange={(e) => setBstInput(e.target.value)}
              />
              <button
                onClick={addBstValues}
                className="px-3 py-1 rounded-xl bg-black text-white hover:bg-gray-800"
              >
                Insert
              </button>
                      ...
                      function walk(n: TreeNode | null) {
                        ...
                        nodes.push(
                          <g key={`n-${n.value}`} transform={`translate(${n.x}, ${n.y})`}>
                            ...
                            <text textAnchor="middle" dy="5" fontSize="12" fill="white">
                              {n.value}
                            </text>
                          </g>
                        );
            ...
            <p className="mt-2 text-xs text-gray-500">
              Enter comma-separated integers and click Insert.
            </p>
          </div>
        </section>
